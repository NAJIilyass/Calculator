...
const Calc = () => {
    ...
    const handleClick = (ch) => {
        if(Number.isInteger(ch)){
            ...
            if (splitValue && splitValue.length > 0 && splitValue[1] && splitValue[1].length === 1) {
                setValue(value.toString().substring(0, value.toString().length - 2));
            }else if(value.toString().length===1 || (value.toString().length===2 && value.toString().charAt(0)==='-')){
                setValue("0")
            }else {
                setValue(value.toString().substring(0, value.toString().length - 1));
            }
        }else if(ch==="clear"){
            setLine("0")
            setValue("0")
            ...
            if(line==="0" || (line && line.charAt(line.length-1)==='=')){
            ...
            if(line==="0"){
                setLine(value+ch)
                setAns(eval((value+ch).substring(0,(value+ch).length-1)).toString())
                setValue(eval((value+ch).substring(0,(value+ch).length-1)).toString())
                ...
                if(clickParenthesis>0){
                    ...
                    if(line.includes("√")){
                        setLine(line+value+ch)
                        setAns(eval((line.replace("√", "Math.sqrt")+value+ch).substring(0,(line.replace("√", "Math.sqrt")+value+ch).length-1)).toString())
                        setValue(eval((line.replace("√", "Math.sqrt")+value+ch).substring(0,(line.replace("√", "Math.sqrt")+value+ch).length-1)).toString())
                    }
                    else{
                        setLine(line+value+ch)
                        setAns(eval((line+value+ch).substring(0,(line+value+ch).length-1)).toString())
                        setValue(eval((line+value+ch).substring(0,(line+value+ch).length-1)).toString())
                    }
                }
            }
            setClick(true)
            ...
            if(line==="0"){
                setLine(value+"*")
                setAns(eval((value+"*").substring(0,(value+"*").length-1)).toString())
                setValue(eval((value+"*").substring(0,(value+"*").length-1)).toString())
            }else if(value===ans && !clickNumber && !line.endsWith(")")){
            ...
            if(clickParenthesis!==0){
                ...
                if(!(line && line.charAt(line.length-1)==='=')){
                    if(value===ans && !clickNumber && !line.endsWith(")")){
                        if(line.includes("√")){
                            setLine(line.substring(0, line.length-1)+"=")
                            setAns(eval((line.replace("√", "Math.sqrt")+value)).toString())
                            setAns1(eval((line.replace("√", "Math.sqrt")+value)).toString())
                            setValue(eval((line.replace("√", "Math.sqrt")+value)).toString())
                        }else{
                            setLine(line.substring(0, line.length-1)+"=")
                        ...
                        setLine(value+"=")
                        setAns1(value)
                    }else if(/^[0-9]+$/.test(line.substring(0, line.length-1)) && ans1!==value && !clickNumber){ //Pour savoir si on est dans le cas quand user change le symbole (de + à = par exemple)
                        setLine(line.substring(0, line.length-1)+"=")
                        ...
                        if(line.includes("√")){
                            setLine(line+"=")
                            setAns(eval((line.replace("√", "Math.sqrt"))).toString())
                            setAns1(eval((line.replace("√", "Math.sqrt"))).toString())
                            setValue(eval((line.replace("√", "Math.sqrt"))).toString())
                        }else{
                            setLine(line+"=")
                            setAns(eval((line)).toString())
    ...
    return ( 
        <div className="bg-white rounded-2xl h-8/12 md:h-11/12 w-11/12 lg:w-9/12 xl:w-2/6 text-base lg:text-2xl min-w-[400px] font-semibold select-none font-montserrat text-blue-800 px-7 py-8">
            <div className="bg-blue-100 h-2/6 pb-4 pt-2 rounded-lg px-2">
                <p id="forScrolling" className="text-right text-lg mr-5 border-3 overflow-x-auto">{line}</p>
                {!syntaxeError && <input 
                    type="number"
                    value={value}
                ...
                />}
                {syntaxeError && <p className="bg-blue-100 text-right font-mono text-blue-950 text-3xl py-4 w-full">Syntaxe Error</p>}
            </div>

            <div className="text-center grid grid-rows-5 gap-5 mt-5">
